function MeasureRuler(dParam){
    ...
    this.initRuler=function(){
        ...
        $("#"+param.wrapperId).on('touchmove',function(e){
            ...
            //超过最左或者最右标尺不可滑动
            if(lf > _limitLeft || limitLeft-(param.max/param.minUnit)*10*param.mult>lf){
                return;
            }
        ...
        $("#"+param.wrapperId).on('touchend',function(e){
            ...
            var lf=$("#"+param.wrapperId+" .rulerScroller").position().left;
            if(lf > _limitLeft){
                $("#"+param.wrapperId+" .rulerScroller").css("left",_limitLeft);
                if(param.callback) param.callback(param.min||0);
                return;
            }else if(limitLeft-(param.max/param.minUnit)*10*param.mult>lf){
                $("#"+param.wrapperId+" .rulerScroller").css("left",limitLeft-(param.max/param.minUnit)*10*param.mult);
            }
            var lf=$("#"+param.wrapperId+" .rulerScroller").position().left;
            ...
            pointerVal=param.minUnit*pointerVal;
                  if(param.callback)  param.callback(pointerVal)
                  if(param.callback2)  param.callback2(pointerVal)
        })
        ...
        for(var z=1;z<setLen;z++){
            if(z*setWidth>pleft) {
                ...
                if(param.min){
                    _limitLeft = limitLeft-(param.min/param.minUnit)*10*param.mult;
                }
                break;
            }
        }
    ...
    this.setIt = function(val){
        $("#"+param.wrapperId+" .rulerScroller").css("left",limitLeft-(val/param.minUnit)*10*param.mult);
    };
