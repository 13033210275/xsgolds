...
use app\home\model\RecommendEnter;
use app\home\model\RecommendSet;
use app\home\model\RecommendMerchant;
...
//        $sql = "select id,name,logo as img,tag,adv_txt,comment_num from xs_merchant where `status`=:status AND type=:type";
        $sql = "`status`=1 AND type=1";
        ...
        if($day && $unit){
            $sql.=" AND (min_date>={$day} OR max_date<={$day})";
        ...
        if($cash!='null' && $cash){
            if(strpos($cash, '-')!==false){
                ...
                $map['min_cash'] = $min_cash*1000;
                $map['max_cash'] = $max_cash*1000;
                $sql.=" AND (min_cash between {$map['min_cash']} and {$map['max_cash']} OR max_cash between {$map['min_cash']} and {$map['max_cash']})";
            }elseif($cash==20 || strpos($cash, '+')!==false){
                $cash = str_replace('+', '', $cash);
                $cash = $cash*1000;
                $sql.=" AND (min_cash >= {$cash} OR max_cash >= {$cash})";
            }else{
                $map['min_cash'] = $cash;
                $map['max_cash'] = $cash;
                $sql.=" AND (min_cash between {$map['min_cash']} and {$map['max_cash']} OR max_cash between {$map['min_cash']} and {$map['max_cash']})";
...
//        $list = \think\Db::query($sql,$map);
        $list = Merchant::field('id,name,logo as img,tag,adv_txt,comment_num,max_cash,max_rate,min_date,max_date,min_day_unit')->where($sql)->order('rise asc')->limit($start,$size)->select();
        ...
        foreach ($list as &$vo) {
            ...
            $vo['max_cash'] = $vo->max_cash?$vo->max_cash:'0';
            $vo['max_rate'] = $vo->max_rate?$vo->max_rate:'0';
            $vo['min_date'] = $vo->min_date?$vo->min_date:'0';
            $vo['max_date'] = $vo->max_date?$vo->max_date:'0';
            $vo['min_day_unit'] = $vo->min_day_unit?$vo->min_day_unit:'日';
    ...
    public function getProductDetail(){
        ...
        $o = new Comment;
        $row['commentTag'] = $o->getCommentTag($row->id);
        finish(0,'',$row);
    }
    ...
    public function getZtProduct(){
        ...
        $list = Merchant::field('id,name,logo as img,tag,adv_txt')->where([
            'zt_id'=>$id,
            'status'=>1
        ])->order('zt_rise asc,rise asc')->limit($start,$size)->select();
        if(!$list){
            finish(2,'数据为空');
        }
        foreach ($list as &$vo) {
    ...
    public function indexNews(){
        ...
        $start = ($page-1)*$size;
        $data =  [];
        $list = Document::order('rise asc,id desc')->field('id,title,img,create_time')->limit($start,$size)->select();
        if(!$list){
            finish(2,'数据为空');
        }
        foreach ($list as $vo) {
...
//                $job = get_rand_jobs();
                ...
                $data[]=[
                    ...
                    'text'=>$city->name.$uname->name.$suffix.','.'申请的<span style="color:#ff2424">'.$cash.'</span>元已经到账'
                ];
            }
        }
        finish(0,'',$data);
    ...
    public function regs(){
        ...
        if(!$code || strlen($code)!=6 || Session::get($phone)!=$code){
            finish(1,'短信验证码错误或已失效');
        }

        $user = new Member;
        $uuid = Member::where('mobile',$phone)->field('id')->find();
        if($uuid){
            cookie('invite',null);
            Session::delete($phone);
            Session::set('uid',$uuid->id);
            Session::set('uname',$phone);
            if($datass=='Android'){
                finish(3);
            }else {
...
//            $ips[] = '113.77.133.'.$i;
//        }
//        $obj->action_ip  = $ips[array_rand($ips)];
    ...
    public function apply(){
        ...
        //检测是否黑名单
        $user = Member::where('id',$user_id)->field('status,golds_black,wd_black,rec_type')->find();
        if(!$user){
            finish(1,'用户不存在');
        }
        if($user['status']!=1){
        ...
        if(strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone')||strpos($_SERVER['HTTP_USER_AGENT'], 'iPad')){
            $sor_code='2219000002';//IOS小树时代浏览器
            if( $user['rec_type']==1){
    ...
    public function getBottomData()
    {
        ...
        $datas = $recommend_enter->getDatasNoStatus(1);
        $recommend_set = new RecommendSet();
        $list = $recommend_set->getDatas($uid, 1);
        $leftUrl = $datas['leftUrl'];
        if ($list) {
            $leftUrl = $list["leftUrl"];
        }
        $datas[1] = array("name"=>$datas['leftName'], "url"=>$leftUrl);
        $datas[2] = array("name"=>$datas['rightName'], "url"=>$datas['rightUrl']);
        finish(0,'',$datas);
    }
    private function _getList($list, $id)
    {
        foreach ($list as $vao) {
            if ($vao->id == $id) {
                return $vao;
...
//        $sql = "select id,name,logo as img,tag,adv_txt,comment_num from xs_merchant where `status`=:status AND type=:type";
        ...
        if($day && $unit){
            $sql.=" AND (m.min_date>={$day} AND max_date<={$day})";
        }

        if($user_type){
            $job = get_jobs($user_type);
            if($job){
                $sql.=" AND m.to_people like '%{$job}%' ";
            }
        }
        if($cash!='null' && $cash){
            if(strpos($cash, '-')!==false){
                ...
                $map['min_cash'] = $min_cash*1000;
                $map['max_cash'] = $max_cash*1000;
                $sql.=" AND (m.min_cash >= {$map['min_cash']}  AND  m.max_cash <= {$map['max_cash']})";
            }elseif($cash==20 || strpos($cash, '+')!==false){
                $cash = str_replace('+', '', $cash);
                $cash = $cash*1000;
                $sql.=" AND min_cash >= {$cash}";
            }else{
                $sql.=" AND m.max_cash <= {$cash}";
            }
        }
        if($loan_type){
            $sql.=" AND m.cate_id={$loan_type} ";
        }
        if($tag_id){
            $tag = get_tags($tag_id);
            $sql.=" AND m.tag like '%{$tag}%' ";
        }

        $fields = 'rm.*,m.id,m.url,m.name,m.max_date,m.min_rate,m.min_day_unit,m.min_date,m.max_cash,m.min_cash,m.tag,m.comment_num,p.path AS img,ms.apply_num,ms.use_sys,ms.sys_score,ms.user_score';
        $list = Db::table('xs_recommend_merchant')
            ->alias('rm')
            ->field($fields)
            ->join('xs_merchant m', 'rm.id=m.id')
            ->join('xs_merchant_score ms', 'm.id=ms.mert_id', 'LEFT')
            ->join('xs_picture p', 'p.id=m.logo', 'LEFT')
            ->where('rm.status=1 AND '.$sql)
            ->order('rm.rise', 'asc')
            ->limit($start, $size)
            ->select();
        ...
        foreach ($list as $key=>$item) {
            $list[$key]['text'] = $item['txt'] ? $item['txt'] : '快来看看';
            $list[$key]['picture']['path'] = $item['img'];
            $list[$key]['star'] = floatval($item['use_sys'] ? $item['sys_score'] : $item['user_score']);
            $list[$key]['number'] = $item['apply_num'] ? $item['apply_num'] : 0;
        }
    ...
    public function getBannerView()
    {
        ...
        $status = 1;
        $adv_type = 1;
        $banner = $adv->bannerList($adv_type, $status);
        finish(0,'',$banner);
    }
    ...
    public function getAllRecommend()
    {
        ...
        foreach ($list as $vo)
        {
            $datas = Merchant::field('id, name, logo as img, tag, adv_txt,txt as text')->where([
                'zt_id'=>$vo->id,
                'status'=>1,
                'is_index'=>1,
               // ])->order('zt_rise asc,rise asc')->select();
            ])->order('sub_rise asc, zt_rise asc')->select();
            if($datas)
            {
                foreach ($datas as &$vao) {
                    $vao->img = $vao->picture->path;
                    unset($vao->picture);
                }
            }
            $vo->datas = $datas;
