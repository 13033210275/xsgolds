 ...
 * @return string
 */
function fdate($sTime, $type = 'normal', $alt = 'false') {
    if (!$sTime)
        return '';
 ...
 * @return  json;
 */
function json($status, $info = "", $data = null) {
    $result ['code'] = $status;
    $result ['msg'] = $info; 
    $result ['data'] = $data;
 ...
 * @param array $data
 * @param string $name
 * @param string|int|array $curr
 * @return string
 */
function bulid_form($type, $data, $name, $curr = "", $field = "", $read = false, $showval = false, $style = "") {
    $html = '';
    if (!$data)
        return $html;
    if ($type == "radio") {
        foreach ($data as $key => $value) {
            ...
            $html.='<label class="mar_r_10"><input data-val="' . $value . '" type="radio" name="' . $name . '" class="' . $name . '" value="' . $tmp . '"';
            if ($curr == $tmp)
                $html.=' checked ';
            $html.=' /> ' . $value . ' </label><span class="wd10"></span>';
        }
    }
    if ($type == "select") {
        $html = '<select ';
        ...
        $html.='name="' . $name . '" class="select ' . $style . '" id="' . $name . '">';
        foreach ($data as $key => $value) {
            if (is_array($value)) {
                $html.="<option  value='{$value[$field[0]]}'";
                if ($curr == $value[$field[0]])
                    $html.=" selected ";
                $html.='> ' . $value[$field[1]] . '</option>';
            }else {
                if ($showval) {
    ...
    if ($type == "checkbox") {
        ...
        foreach ($data as $key => $value) {
            if (is_array($value)) {
                $html.='<label><input class="ckbox" type="checkbox" name="' . $name . '" value="' . $value[$field[0]] . '"';
                if (is_array($curr) && in_array($value[$field[0]], $curr))
                    $html.=' checked ';
                $html.=' /> ' . $value[$field[1]] . ' </label><span class="wd10"></span>';
            }else {
                $val = $key;
                if ($showval) {
                    $val = $value;
                }
                $html.='<label><input class="ckbox" type="checkbox" name="' . $name . '" value="' . $val . '"';
                if ($field == "enum") {
                    if (($curr & $val) == $val) {
                        $html.=' checked ';
                    }
                } else {
                    if ($curr && in_array($val, $curr))
                        $html.=' checked ';
                }
                $html.=' /> ' . $value . ' </label><span class="wd10"></span>';
            }
...
function getCommentType($type_id=null){
    $type = [
        0=>'全部',
        1=>'已申请',
        2=>'出额度',
