...
class Admin extends Controller {
    ...
    public function __construct() { 
        ...
        if (!$config) {
            ...
            $config['admin_view_path'] = 'default';
     ...
     * @return boolean
     */
    final protected function checkRule($rule, $type = Menu::rule_url, $mode = 'url',$pid=null) {
        static $Auth = null;
        if (!$Auth) {
            $Auth = new \com\Auth();
        }
      
        if (!$Auth->check($rule, UID, $type, $mode,'or',$pid)) {
            return false;
        }
     ...
     * @author xsgolds
     */
    protected function forbid($model, $where = array(), $msg = array('success' => '状态禁用成功！', 'error' => '状态禁用失败！'),$field="status",$val=0) {
     ...
     * @author xsgolds
     */
    protected function resume($model, $where = array(), $msg = array('success' => '状态恢复成功！', 'error' => '状态恢复失败！'),$field='status',$val=1) {
        $data = array($field =>$val);
        $this->editRow($model, $data, $where, $msg);
    }
    ...
    protected function _delete($model, $where = array(), $msg = array('success' => '删除成功！', 'error' => '删除失败！')) {
        ...
        if (db($model)->where($where)->delete() !== false) {
            $this->success($msg['success'], $msg['url'], $msg['ajax']);
        }else{
            $this->error($msg['error'], $msg['url'], $msg['ajax']);
        }
...
//                // 判断主菜单权限
//                if (!IS_ROOT && !$this->checkRule(strtolower($model_name . '/' . $item['url']), Menu::rule_main, null)) {
...
//            }
            ...
            if ($pid) {
                ...
                foreach ($menus['main'] as $sak => $item) {
                    // 获取当前主菜单的子菜单项
                    if ($item['id'] == $nav['id']) {
                        $menus['main'][$sak]['class'] = 'current';
                        //生成child树
                        ...
                        if (!IS_ROOT) {
                            ...
                            foreach ($second_urls as $key => $to_check_url) {
                                ...
                                if ($this->checkRule($rule, Menu::rule_url, null,0)){
                                    if(!$indexUrl){
                                        $indexUrl = $to_check_url;
                                    }
                                    $to_check_urls[] = $to_check_url;
                                    if($item['url']==$to_check_url){
                                        $isIndex = true;
                                    }
                                }
                            }
                            if(!$isIndex){
                                $menus['main'][$sak]['url'] = $indexUrl;
                            }
                        }
                        ...
                        foreach ($groups as $g) {
                            ...
                            $menuList = \think\Db::name('Menu')->where($map)->field('id,pid,sort,title,url,tip')->order('sort asc')->select();
    ...
    public function handle($obj,$id,$cmd){
        $f1 = 0;
        switch ($cmd) {
            case 'forbid':
                $f1 = $obj->where('id',$id)->update([
                    'status'=>0
                ]);
                break;
            case 'resume':
